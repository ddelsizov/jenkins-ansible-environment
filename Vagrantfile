...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "jenkins" do |jenkins|
    ...
    ansible.galaxy_role_file = "/vagrant/master-requirements.yml"
    ansible.galaxy_roles_path = "/etc/ansible/roles"
    ansible.inventory_path = "/vagrant/inventory"
    ansible.playbook = "/vagrant/master-playbook.yml"
    end
   end
   config.vm.define "slave" do |slave|
	...
	slave.vm.synced_folder "./vagrant/jenkins/slave", "/vagrant", mount_options: ["dmode=755", "fmode=755"]
    ...
    ansible.limit = "all"
    ansible.galaxy_command = "sudo ansible-galaxy install --role-file=%{role_file} --roles-path=%{roles_path} --force"
    ansible.galaxy_role_file = "/vagrant/slave-requirements.yml"
    ansible.galaxy_roles_path = "/etc/ansible/roles"
    ansible.inventory_path = "/vagrant/inventory"
    ansible.playbook = "/vagrant/slave-playbook.yml"
   end
 end
 end